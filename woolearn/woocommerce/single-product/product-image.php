<?php
   ...
   ?>
<div class="col-md-6">
   <div class="product-slider shadow-around">
         ...
         <div class="woocommerce-product-gallery <?php echo 'woocommerce-product-gallery--columns-' . sanitize_html_class( $columns ) . ' columns-' . sanitize_html_class( $columns ); ?> images">
            <?php
               ...
               if ( has_post_thumbnail() ) {
               	echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', '<figure data-zoom-image="' . get_the_post_thumbnail_url( $post->ID, 'shop_thumbnail' ) . '" class="zoom" data-thumb="' . get_the_post_thumbnail_url( $post->ID, 'shop_thumbnail' ) . '" class="woocommerce-product-gallery__image">' . get_the_post_thumbnail( $post->ID, 'shop_single', $attributes ) . '</figure>' );
               } else {
